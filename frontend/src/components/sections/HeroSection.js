...
const HeroContent = ({ onScrollToBrands, brandsCount }) => (
  <div className="grid lg:grid-cols-2 gap-16 lg:gap-24 items-center">
    <div>
      {/* Premium Badge */}
      <div className="inline-flex items-center gap-4 mb-12 group animate-fade-in-up">
        <div className="h-[1px] w-12 bg-gradient-to-r from-transparent to-slate-600 group-hover:to-blue-400 transition-colors duration-700"></div>
        <div className="flex items-center gap-3 border border-slate-700 group-hover:border-blue-900 px-5 py-2.5 transition-all duration-700">
          <Shield className="w-4 h-4 text-blue-400" />
          <span className="text-slate-400 text-xs font-semibold tracking-[0.3em] uppercase">Premium Quality</span>
        </div>
        <div className="h-[1px] w-12 bg-gradient-to-l from-transparent to-slate-600 group-hover:to-blue-400 transition-colors duration-700"></div>
      </div>

      {/* Main Headline */}
      <div className="mb-10">
        <h1 className="text-7xl sm:text-8xl lg:text-9xl font-thin text-white mb-4 tracking-tighter leading-[0.85] animate-fade-in-up" style={{ animationDelay: '0.1s' }}>
          DRIVE
        </h1>
        <h2 className="text-5xl sm:text-6xl lg:text-7xl font-light mb-4 tracking-wide animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
        ...
        <div className="flex items-center gap-4 mt-6 animate-fade-in-up" style={{ animationDelay: '0.3s' }}>
          <div className="h-[2px] w-20 bg-gradient-to-r from-blue-400 to-transparent"></div>
          <div className="w-2 h-2 border border-blue-400 rotate-45"></div>
        </div>
      </div>

      {/* Description */}
      <p className="text-slate-400 text-lg lg:text-xl font-light leading-relaxed max-w-xl mb-12 animate-fade-in-up" style={{ animationDelay: '0.4s' }}>
        ...
        {[
          ...
          <div key={title} className="group animate-fade-in-up" style={{ animationDelay: delay }}>
            <div className="flex items-start gap-4">
              <div className="w-14 h-14 border border-slate-700 group-hover:border-blue-500 flex items-center justify-center transition-all duration-500 group-hover:rotate-90">
                <Icon className="w-6 h-6 text-slate-600 group-hover:text-blue-400 transition-all duration-500 group-hover:-rotate-90" />
              </div>
              <div>
                <div className="text-white font-light text-lg mb-1">{title}</div>
                <div className="text-slate-500 text-sm font-light">{subtitle}</div>
              </div>
            </div>
          </div>
        ))}
...
const ActionButtons = ({ onScrollToBrands }) => (
  <div className="flex flex-col sm:flex-row gap-4 mb-16">
    <button
      ...
      className="group relative bg-white hover:bg-slate-100 text-black px-10 py-5 font-light text-lg transition-all duration-500 overflow-hidden"
    >
    ...
    <a 
      ...
      className="group relative border border-slate-600 hover:border-slate-500 text-white px-10 py-5 font-light text-lg transition-all duration-500 overflow-hidden"
    >
      <span className="relative z-10 flex items-center justify-center gap-3 tracking-wide">
    ...
    <div className="flex items-center gap-12">
      {stats.map((stat, index) => (
        <React.Fragment key={stat.label}>
          <div className="group">
            <div className="text-4xl font-thin text-white mb-2 group-hover:text-blue-400 transition-colors duration-500 tracking-tight">
              {stat.value}
            </div>
            <div className="h-[1px] w-12 bg-slate-700 group-hover:w-16 group-hover:bg-blue-500 transition-all duration-500 mb-2"></div>
            <div className="text-slate-500 text-sm font-light tracking-wider uppercase">{stat.label}</div>
          </div>
          {index < stats.length - 1 && <div className="w-[1px] h-16 bg-slate-700"></div>}
        </React.Fragment>
      ))}
    </div>
  );
...
const ScrollIndicator = () => (
  <div className="absolute bottom-12 left-1/2 transform -translate-x-1/2 z-20 animate-fade-in-up" style={{ animationDelay: '1s' }}>
    <div className="flex flex-col items-center gap-3 group cursor-pointer">
      <span className="text-xs text-slate-500 font-light tracking-[0.3em] uppercase">Scroll</span>
      <div className="w-[1px] h-16 bg-gradient-to-b from-slate-600 to-transparent group-hover:from-blue-500 transition-colors duration-700"></div>
      <ChevronDown className="w-5 h-5 text-slate-600 group-hover:text-blue-400 animate-bounce transition-colors duration-700" />
    </div>
  </div>
);
